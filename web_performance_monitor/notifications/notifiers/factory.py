...
class NotificationFactory:
    ...
    def register_notifier_type(self, notifier_type: str, notifier_class: Type[BaseNotifier]) -> None:
        """注册新的通知器类型，支持扩展
        ...
        self.logger.info(f"注册通知器类型: {notifier_type} -> {notifier_class.__name__}")
    ...
    def create_custom_notifiers(self, notifier_configs: List[Dict[str, Any]]) -> List[BaseNotifier]:
        """根据自定义配置创建通知器列表
    ...
    def validate_all_configs(self) -> Dict[str, bool]:
        ...
        if self.config.enable_local_file:
            try:
                ...
                results['local_file'] = notifier.validate_config() if hasattr(notifier, 'validate_config') else True
            except Exception as e:
        ...
        if self.config.enable_mattermost:
            try:
                ...
                results['mattermost'] = notifier.validate_config() if hasattr(notifier, 'validate_config') else True
            except Exception as e:
