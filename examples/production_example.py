...
def create_production_app():
    ...
    # 如果环境变量未设置，使用生产环境默认值
    if not any(
        os.getenv(key) for key in ['WPM_THRESHOLD_SECONDS', 'WPM_ALERT_WINDOW_DAYS']):
        config = Config(
            # 生产环境推荐配置
...
def setup_production_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('/tmp/log/app.log') if os.path.exists(
                '/tmp/log') else logging.NullHandler()
        ]
    )
