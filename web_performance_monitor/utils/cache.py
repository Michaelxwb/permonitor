...
class CacheManager:
    ...
    def generate_alert_key(self, endpoint: str) -> str:
        ...
        try:
            # 仅使用接口端点生成缓存键
            cache_key = hashlib.md5(endpoint.encode('utf-8')).hexdigest()
        ...
        with self._lock:
            try:
                ...
                return {
                    ...
                    'recent_7d': recent_7d,
                    'oldest_entry': min((entry.timestamp for entry in self._cache.values()), default=None),
                    'newest_entry': max((entry.timestamp for entry in self._cache.values()), default=None)
                }
